...
| [`brief/07-keskeny-allapotok.md`](brief/07-keskeny-allapotok.md) | **this round**: what the pane *bodies* do as they narrow — at 958 · 638 · 382, the widths that actually occur |
...
1. **Narrow states — asked for in `brief/07`.** Everything is drawn at 1600 × 1000; thirteen of the
   31 pane screens have no narrow treatment at all. The widths that actually occur are
   958 · 638 · 572 · 382, and 638 is the everyday one.
2. **Five consolidations** proposed in the review: nineteen pane kinds down to eleven, with no
   entrance to the product removed. Not yet reflected in the screens.
3. **Six prose lines** in `brief/plans/` now contradict what was decided.

## Settled since the review

- **The cluster rail defaults to zero width**, as `06-panes/29` argued: `⌘\` cycles three widths, it
  slides in for the length of a drag, and the one job only it could do — warning about a cluster you
  are not looking at — becomes a tab-bar chip that exists only while it is true.
- **The narrow states get their own round** rather than being invented during the build.
